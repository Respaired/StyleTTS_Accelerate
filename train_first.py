...
def main(config_path):
    ...
    ddp_kwargs = DistributedDataParallelKwargs(find_unused_parameters=True)
    accelerator = Accelerator(project_dir=log_dir, split_batches=True, kwargs_handlers=[ddp_kwargs], mixed_precision='fp16')    
    if accelerator.is_main_process:
        writer = SummaryWriter(log_dir + "/tensorboard")
        ...
        if accelerator.is_main_process:
          ...
          if epoch % saving_epoch == 0:
              ...
              api.upload_file(
                    ...
                    path_in_repo="StyleTTS_Accelerate/Models/Anispeech_with_DIF/",
                    repo_id="Respair/stts_test_ckpt",
                    repo_type="model",
                    token=hf_token
                )
              print(f"Uploaded {save_path} to Hugging Face Hub.")
